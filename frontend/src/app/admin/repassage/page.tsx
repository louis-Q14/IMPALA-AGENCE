...
import {
  ...
  ArrowPathIcon, XMarkIcon, ScissorsIcon, ExclamationTriangleIcon, TrashIcon,
...
const defaultPricingConfig: PricingConfig = {
  small: { label: "Petit lot (< 10 pieces)", price: "15", duration: 1 },
  medium: { label: "Lot moyen (10-25 pieces)", price: "28", duration: 2 },
  large: { label: "Grand lot (> 25 pieces)", price: "45", duration: 4 },
  unite: "CDF",
};
...
const QUANTITY_LABELS: Record<string, string> = {
  small: "Petit lot (< 10 pieces)",
  medium: "Lot moyen (10-25 pieces)",
  large: "Grand lot (> 25 pieces)",
};
...
const saveDiscountToAPI = async (email: string, discountType: string, percent: number, startsAt: string, endsAt: string) => {
  try {
    ...
    await fetch(`${API}/admin/repassage/discounts`, { method: "PUT", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify({ email, discountType, percent, startsAt: toISO(startsAt), endsAt: toISO(endsAt) }) });
  } catch (e) { console.error(e); }
};
const removeDiscountFromAPI = async (email: string) => {
  try { const token = getToken(); if (!token) return; await fetch(`${API}/admin/repassage/discounts/${encodeURIComponent(email)}`, { method: "DELETE", headers: { Authorization: `Bearer ${token}` } }); } catch (e) { console.error(e); }
};
const saveConfigToAPI = async (config: PricingConfig) => {
  try { const token = getToken(); if (!token) return; await fetch(`${API}/admin/repassage/config`, { method: "PUT", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify({ pricingConfig: config }) }); } catch (e) { console.error(e); }
};
const saveBookingToAPI = async (b: Booking) => {
  try { const token = getToken(); if (!token) return; await fetch(`${API}/admin/repassage/bookings`, { method: "PUT", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify({ email: b.email, fullName: b.client, address: b.address, quantity: b.quantity, clothingTypes: b.clothingTypes || [], duration: b.duration, date: b.date || "", time: b.time || "", amount: b.amount || "", notes: b.notes || "", status: b.status }) }); } catch (e) { console.error(e); }
};

export default function AdminRepassage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        const r = await fetch(`${API}/admin/repassage/bookings`, { headers: h });
        if (r.ok) {
          ...
          if (rows.length > 0) setBookings(rows.map((row: Record<string, unknown>, i: number) => ({
            ...
            quantity: ((row.quantity as string) || "small") as "small" | "medium" | "large",
            clothingTypes: (row.clothing_types as string[]) || [],
            duration: (row.duration as number) || 1, date: (row.date as string) || "",
      ...
      try {
        const r = await fetch(`${API}/admin/repassage/config`, { headers: h });
      ...
      try {
        const r = await fetch(`${API}/admin/repassage/discounts`, { headers: h });
  ...
  const calcAmount = (q: "small" | "medium" | "large", cfg?: PricingConfig) =>
    `${(cfg || pricingConfig)[q].price} ${currency}/seance`;
  const calcFinalAmount = (b: Booking, cfg?: PricingConfig) => {
    const c = cfg || pricingConfig; let base = parseInt(c[b.quantity].price) || 0;
  ...
  const deleteBooking = (b: Booking) => {
    ...
    (async () => { try { const t = getToken(); if (!t) return; await fetch(`${API}/admin/repassage/bookings/${encodeURIComponent(b.email)}`, { method: "DELETE", headers: { Authorization: `Bearer ${t}` } }); } catch (e) { console.error(e); } })();
  };

  const filtered = bookings.filter(b => { if (filter !== "all" && b.status !== filter) return false; const q = search.toLowerCase(); if (q && !b.client.toLowerCase().includes(q) && !b.email.toLowerCase().includes(q) && !b.address.toLowerCase().includes(q)) return false; return true; });
  ...
  return (
    <>
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-bold text-[var(--text-primary)]">Repassage a domicile</h1>
          <p className="text-sm text-[var(--text-muted)]">Gestion des reservations de repassage</p>
        </div>
      </div>

      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
        {[
          { label: "Total reservations", value: counts.all, icon: ScissorsIcon, color: "bg-violet-500" },
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-5">
        <div className="relative flex-1">
          ...
          <input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder="Rechercher par client, email ou adresse..." className="w-full pl-9 pr-4 py-2.5 rounded-xl bg-[var(--bg-card)] border border-[var(--border-color)] text-sm text-[var(--text-primary)] placeholder:text-[var(--text-muted)] focus:outline-none focus:ring-2 focus:ring-violet-500/40" />
        </div>
        <div className="flex gap-2 flex-wrap">
          {(["all", "pending", "confirmed", "completed", "cancelled"] as const).map(s => (
            <button key={s} onClick={() => setFilter(s)} className={`px-3 py-2 rounded-xl text-sm font-medium transition-all ${filter === s ? "bg-violet-600 text-white" : "bg-[var(--bg-card)] border border-[var(--border-color)] text-[var(--text-secondary)] hover:bg-[var(--bg-hover)]"}`}>
              {s === "all" ? "Tous" : STATUS_CONFIG[s].label}<span className="ml-1.5 text-xs opacity-70">{counts[s as keyof typeof counts]}</span>
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="rounded-2xl bg-[var(--bg-card)] border border-[var(--border-color)] overflow-hidden">
        {filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 text-center">
            <div className="w-14 h-14 rounded-2xl bg-violet-500/10 flex items-center justify-center mb-4"><ScissorsIcon className="w-7 h-7 text-violet-500" /></div>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-[var(--border-color)] bg-[var(--bg-tertiary)]">
                  {["Client", "Quantite", "Date & Heure", "Duree", "Montant", "Statut", "Actions"].map(h => (
                  ...
                  <tr key={b.id} className="hover:bg-[var(--bg-hover)] transition-all">
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-3">
                        <div className="w-9 h-9 rounded-full bg-violet-100 dark:bg-violet-900/30 flex items-center justify-center text-violet-600 font-bold text-xs">{b.client.split(" ").map((n: string) => n[0]).join("").slice(0, 3)}</div>
                    ...
                    <td className="px-4 py-3 text-sm text-[var(--text-secondary)]">{QUANTITY_LABELS[b.quantity] ?? b.quantity}</td>
                    ...
                    <td className="px-4 py-3 text-sm font-medium text-[var(--text-primary)]">{b.amount}</td>
          ...
          <div className="relative w-full max-w-2xl max-h-[90vh] flex flex-col rounded-3xl bg-white/20 dark:bg-gray-900/20 backdrop-blur-2xl shadow-[0_4px_20px_rgba(0,0,0,0.08)] dark:shadow-[0_4px_20px_rgba(0,0,0,0.25)] border-[0.5px] border-gray-300/50 dark:border-gray-500/30"
            ...
            <div className="shrink-0 px-6 pt-5 pb-4 cursor-grab active:cursor-grabbing select-none bg-white dark:bg-gray-900 rounded-t-3xl border-b border-[0.5px] border-gray-300/30 dark:border-gray-500/20" onMouseDown={onDragStart}>
              <div className="flex items-center gap-4">
                <div className={`w-14 h-14 rounded-2xl flex items-center justify-center font-bold text-lg shadow-lg ${selected.status === "completed" ? "bg-gradient-to-br from-emerald-400 to-green-600 text-white" : selected.status === "pending" ? "bg-gradient-to-br from-amber-400 to-orange-500 text-white" : selected.status === "confirmed" ? "bg-gradient-to-br from-violet-400 to-violet-600 text-white" : "bg-gradient-to-br from-red-400 to-red-600 text-white"}`}>
                  {selected.client.split(" ").map((n: string) => n[0]).join("").toUpperCase().slice(0, 3)}
                </div>
                <div className="flex-1 min-w-0"><h3 className="text-lg font-bold text-gray-900 dark:text-white truncate">{selected.client}</h3><p className="text-sm text-gray-500 dark:text-gray-400">{selected.email}</p></div>
            ...
            <div className="px-6 py-4 space-y-4 overflow-y-auto flex-1 min-h-0">
              {modalTab === "details" && (<>
                {selected.status === "pending" && (
                  <div className="flex items-center gap-3 p-4 rounded-2xl bg-amber-50/80 dark:bg-amber-900/20 border border-amber-200/50">
                    <div className="w-10 h-10 rounded-xl bg-amber-100 flex items-center justify-center"><ExclamationTriangleIcon className="w-5 h-5 text-amber-600" /></div>
                ...
                <div>
                  ...
                  <div className="divide-y divide-white/30 dark:divide-white/5">
                    {[{ label: "Quantite", value: QUANTITY_LABELS[selected.quantity] ?? selected.quantity }, { label: "Duree", value: `${selected.duration}h` }, { label: "Date et heure", value: `${selected.date} a ${selected.time}` }, { label: "Montant", value: selected.amount }].map(item => (
                      <div key={item.label} className="flex justify-between items-center px-4 py-2.5"><span className="text-sm text-gray-500 dark:text-gray-400">{item.label}</span><span className="text-sm font-medium text-gray-900 dark:text-white">{item.value}</span></div>
                    ))}
                  </div>
                </div>
                {selected.clothingTypes && selected.clothingTypes.length > 0 && (
                  <div>
                    <div className="flex items-center gap-2 px-4 py-2.5 rounded-lg bg-gray-200/40 dark:bg-white/8 mb-1"><span className="text-sm font-semibold text-gray-800 dark:text-white">Types de vetements</span></div>
                    <div className="flex flex-wrap gap-2 px-4 py-2">
                      {selected.clothingTypes.map(t => (<span key={t} className="px-2 py-0.5 rounded-full bg-violet-500/10 text-violet-600 dark:text-violet-400 text-xs">{t}</span>))}
                    </div>
                  </div>
                )}
                <div>
                  <div className="flex items-center gap-2 px-4 py-2.5 rounded-lg bg-gray-200/40 dark:bg-white/8 mb-1"><span className="text-sm font-semibold text-gray-800 dark:text-white">Adresse</span></div>
                  <div className="px-4 py-2.5"><span className="text-sm text-gray-900 dark:text-white">{selected.address || "Non renseignee"}</span></div>
                </div>
              ...
              {modalTab === "gerence" && draft && (<>
                <div>
                  <div className="flex items-center gap-2 px-4 py-2.5 rounded-lg bg-gray-200/40 dark:bg-white/8 mb-1"><span className="text-sm font-semibold text-gray-800 dark:text-white">Changer la quantite</span></div>
                  <div className="grid grid-cols-3 gap-3 px-4 py-3">
                    {(["small", "medium", "large"] as const).map(q => {
                      const info = {
                        small: { label: "Petit lot", color: "from-gray-400 to-gray-500", desc: `${draftConfig.small.price} ${currency}`, sub: `${draftConfig.small.duration}h` },
                        medium: { label: "Lot moyen", color: "from-violet-400 to-violet-600", desc: `${draftConfig.medium.price} ${currency}`, sub: `${draftConfig.medium.duration}h` },
                        large: { label: "Grand lot", color: "from-purple-400 to-purple-600", desc: `${draftConfig.large.price} ${currency}`, sub: `${draftConfig.large.duration}h` },
                      };
                      const isActive = draft.quantity === q;
                      return (
                        <button key={q} onClick={() => { if (!isActive) { setDraft({ ...draft, quantity: q, duration: draftConfig[q].duration, amount: calcAmount(q, draftConfig) }); setSaved(false); } }}
                          className={`relative p-3 rounded-xl border text-center transition-all ${isActive ? "border-white/40 dark:border-white/20 bg-white/20 dark:bg-white/10 ring-2 ring-violet-400/50" : "border-white/20 dark:border-white/5 hover:bg-white/10"}`}>
                          {isActive && <div className="absolute -top-2 -right-2 w-5 h-5 rounded-full bg-violet-500 flex items-center justify-center"><span className="text-white text-xs">v</span></div>}
                          <div className={`w-8 h-8 mx-auto mb-2 rounded-lg bg-gradient-to-br ${info[q].color} flex items-center justify-center`}><span className="text-white text-xs font-bold">{info[q].label[0]}</span></div>
                          <p className="text-sm font-semibold text-gray-900 dark:text-white">{info[q].label}</p>
                          <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{info[q].desc}</p>
                          <p className="text-[10px] text-gray-400 dark:text-gray-500 mt-0.5">{info[q].sub}</p>
                        </button>
                      );
                    })}
                  </div>
                    ...
                    {draft.discount ? (
                      ...
                      <>
                        <div className="grid grid-cols-2 gap-2">
                          {[{ label: "Remise 10%", percent: 10, type: "Remise fidelite" }, { label: "Remise 20%", percent: 20, type: "Remise parrainage" }, { label: "Remise 30%", percent: 30, type: "Promo speciale" }, { label: "Remise 50%", percent: 50, type: "Offre exceptionnelle" }].map(promo => (
                            <button key={promo.label} onClick={() => { const w = { ...draft, discount: { type: promo.type, percent: promo.percent, startsAt: new Date(promoStart).toLocaleDateString("fr-FR"), endsAt: new Date(promoEnd).toLocaleDateString("fr-FR") } }; setDraft({ ...w, amount: calcFinalAmount(w, draftConfig) }); setSaved(false); }}
                              className="p-2.5 rounded-xl bg-white/10 border border-white/15 hover:bg-amber-500/10 hover:border-amber-400/30 transition-all text-left">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          <div><label className="block text-[10px] font-medium text-gray-500 mb-1">Debut</label><input type="date" value={promoStart} onChange={e => setPromoStart(e.target.value)} className="w-full px-3 py-2 rounded-xl bg-white/10 border border-white/20 text-sm text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-violet-400/50" /></div>
                          <div><label className="block text-[10px] font-medium text-gray-500 mb-1">Fin</label><input type="date" value={promoEnd} onChange={e => setPromoEnd(e.target.value)} className="w-full px-3 py-2 rounded-xl bg-white/10 border border-white/20 text-sm text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-violet-400/50" /></div>
                        </div>
                      </>
                    )}
                  </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-3 gap-3 px-4 py-3">
                    {[{ label: "Montant", value: draft.amount, color: "from-emerald-400 to-green-500" }, { label: "Duree", value: `${draft.duration}h`, color: "from-violet-400 to-violet-500" }, { label: "Date", value: draft.date || "-", color: "from-purple-400 to-purple-500" }].map(s => (
            ...
            <div className="shrink-0 px-6 py-4 border-t border-[0.5px] border-gray-300/30 dark:border-gray-500/20 bg-white/10 dark:bg-gray-900/10 rounded-b-3xl flex gap-3">
              {modalTab === "gerence" ? (
                <>
                  <button onClick={() => { if (draft && draftConfig) { const final = { ...draft, amount: calcFinalAmount(draft, draftConfig) }; setBookings(prev => prev.map(b => b.id === final.id ? final : b)); setSelected(final); setPricingConfig(draftConfig); setSaved(true); if (final.discount) saveDiscountToAPI(final.email, final.discount.type, final.discount.percent, final.discount.startsAt, final.discount.endsAt); else removeDiscountFromAPI(final.email); saveConfigToAPI(draftConfig); saveBookingToAPI(final); } }}
                    className={`flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl text-white font-medium transition-all text-sm shadow-lg ${saved ? "bg-gradient-to-r from-emerald-500 to-green-600" : "bg-gradient-to-r from-violet-500 to-purple-600 hover:from-violet-600 hover:to-purple-700"}`}>
