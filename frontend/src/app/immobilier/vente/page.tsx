...
export default function ImmobilierVentePage() {
  ...
  useEffect(() => {
    const loadAds = async () => {
      try {
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:5000/api"}/real-estate/ads?limit=50&type=sale`,
          { cache: "no-store" }
        );
        if (!res.ok) return;
    ...
    loadAds();
    // Check subscription from stored user
    try {
  ...
  const filteredAds = ads.filter((ad) => {
    if (ad.ad_type !== "sale") return false;
      ...
      <div className="bg-[var(--bg-secondary)] border-b border-[var(--border-color)]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          <div className="flex items-center gap-2 text-sm text-[var(--text-muted)] mb-4">
            ...
            <span>/</span>
            <span className="text-[var(--text-primary)]">Vente</span>
          </div>
          <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6">
            <div>
              ...
              <p className="mt-2 text-[var(--text-secondary)]">
                {filteredAds.length} bien{filteredAds.length !== 1 ? "s" : ""} à vendre disponible{filteredAds.length !== 1 ? "s" : ""}
              </p>
            </div>
            {canPublish && (
            <Link
        ...
        {showFilters && (
          <div className="mb-8 p-6 rounded-2xl bg-[var(--bg-card)] border border-[var(--border-color)]">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              <div>
                <label className="block text-sm font-medium text-[var(--text-secondary)] mb-2">Prix min ($)</label>
                <input
                  type="number"
                  placeholder="0 FC"
              ...
              <div>
                <label className="block text-sm font-medium text-[var(--text-secondary)] mb-2">Prix max ($)</label>
                <input
                  type="number"
                  placeholder="1 000 000 FC"
                  className="w-full px-4 py-2.5 rounded-lg bg-[var(--bg-input)] border border-[var(--border-color)]
                    text-[var(--text-primary)] focus:outline-none focus:ring-2 focus:ring-primary/50"
                />
              </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredAds.length === 0 ? (
              <div className="col-span-3 py-20 text-center">
                ...
                <p className="text-[var(--text-secondary)] text-lg font-medium">Aucun bien à vendre pour le moment</p>
