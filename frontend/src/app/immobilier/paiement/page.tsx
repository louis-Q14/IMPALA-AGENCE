...
export default function PaiementImmobilier() {
  ...
  useEffect(() => {
    ...
    if (!token) {
      router.push("/connexion?redirect=/immobilier/paiement");
      return;
    }
    // Si abonnement deja actif, aller directement au formulaire
    Promise.all([
      fetch(`${API}/subscriptions/mine`, { headers: { Authorization: `Bearer ${token}` } }).then((r) => r.json()).catch(() => []),
      fetch(`${API}/auth/me`, { headers: { Authorization: `Bearer ${token}` } }).then((r) => r.json()).catch(() => ({})),
    ]).then(([subs, me]) => {
      const hasSubRow = Array.isArray(subs) && subs.some((s: { plan_type: string }) =>
        ["real_estate_pro", "immobilier", "complet"].includes(s.plan_type)
      );
      const services: { service: string; status: string }[] = Array.isArray(me?.services) ? me.services : [];
      const hasService = services.some((s) => ["real_estate", "immobilier"].includes(s.service) && s.status === "active");
      if (hasSubRow || hasService) router.push("/immobilier/publier");
    });
    fetch(`${API}/tarifs-frais/public?service=immobilier,general&type=abonnement`)
      .then((r) => r.json())
      .then((data) => {
        const arr = Array.isArray(data) ? data : [];
        setTarif(arr[0] ?? { id: 0, nom: "Frais d annonce Immobilier", montant: "50000", unite: "CDF", description: null });
      })
      .catch(() => setTarif({ id: 0, nom: "Frais d annonce Immobilier", montant: "50000", unite: "CDF", description: null }))
    ...
    try {
      ...
      const res = await fetch(`${API}/subscriptions/manual`, {
        ...
        body: JSON.stringify({
          ...
          amount: tarif ? parseFloat(tarif.montant) : 50000,
        }),
      });
      if (!res.ok) {
  ...
  const montant = tarif ? Math.round(parseFloat(tarif.montant)).toLocaleString("fr-FR") : "50 000";
      ...
      <div className="min-h-screen bg-[var(--bg-primary)]">
        <div className="max-w-lg mx-auto px-4 py-20 text-center">
          ...
          <p className="text-[var(--text-secondary)] mb-6">
            Votre abonnement <strong>Immobilier</strong> est actif pour 30 jours. Vous pouvez maintenant publier vos annonces.
          </p>
          <div className="p-4 rounded-2xl bg-[var(--bg-card)] border border-[var(--border-color)] text-left mb-8 space-y-3 text-sm">
            <div className="flex justify-between">
              ...
              <span className="font-medium text-[var(--text-primary)]">Immobilier</span>
            </div>
            <div className="flex justify-between">
            ...
            <div className="flex justify-between border-t border-[var(--border-color)] pt-3">
              ...
              <span className="font-bold text-blue-500">{montant} {unite}</span>
            </div>
          </div>
          <div className="my-6 p-6 rounded-2xl border-2 border-dashed border-blue-500/30 bg-blue-500/5">
            <QrCodeIcon className="w-16 h-16 text-blue-500 mx-auto mb-2" />
          ...
          <button onClick={() => router.push("/immobilier/publier")}
            className="w-full py-3.5 rounded-xl bg-gradient-to-r from-blue-500 to-indigo-600 text-white font-semibold shadow-lg hover:opacity-90 transition-all mb-3">
            Publier mon annonce maintenant
          </button>
          <Link href="/immobilier" className="block text-sm text-[var(--text-muted)] hover:text-primary transition-colors">
            Retour aux annonces
          </Link>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-[var(--bg-primary)]">
      <div className="bg-[var(--bg-card)] border-b border-[var(--border-color)]">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <Link href="/immobilier" className="inline-flex items-center gap-2 text-sm text-[var(--text-muted)] hover:text-primary transition-colors mb-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-[var(--text-primary)]">Activer l abonnement Immobilier</h1>
              <p className="text-sm text-[var(--text-secondary)]">Publiez vos annonces de vente et location immobiliere</p>
            </div>
          </div>
        </div>
      </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="p-6 rounded-2xl bg-[var(--bg-card)] border border-[var(--border-color)]">
              ...
              <ul className="space-y-3">
                {[
                  "Publication d annonces vente et location immobilieres",
                  "Visibilite aupres d acheteurs et locataires qualifies",
                  "Photos, plans, diagnostics et dossier complet",
                  "Accompagnement juridique IMPALA inclus",
                  "Tableau de bord de suivi de vos annonces",
                ].map((item) => (
                ...
                {methods.map((m) => {
                  ...
                  return (
                    <button key={m.id} onClick={() => setPaymentMethod(m.id)}
                      className={`w-full flex items-center gap-4 p-4 rounded-xl border-2 text-left transition-all ${isActive ? "border-blue-500 bg-blue-500/5 shadow-lg shadow-blue-500/10" : "border-[var(--border-color)] hover:border-[var(--border-hover)]"}`}>
                      ...
                      <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0 ${isActive ? "border-blue-500" : "border-[var(--border-color)]"}`}>
              ...
              <div className="p-6 rounded-2xl bg-[var(--bg-card)] border border-[var(--border-color)] shadow-lg">
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between text-[var(--text-secondary)]">
                    <span>Abonnement Immobilier</span>
                    <span>30 jours</span>
                  </div>
                  <div className="flex justify-between font-bold text-base text-[var(--text-primary)] pt-3 border-t border-[var(--border-color)]">
                    <span>Total</span>
                    <span className="text-blue-500">{montant} {unite}</span>
                  </div>
                </div>
                <div className="mt-4 p-3 rounded-xl bg-blue-500/5 border border-blue-500/20 flex items-start gap-2">
                  <LockClosedIcon className="w-4 h-4 text-blue-500 flex-shrink-0 mt-0.5" />
                  <p className="text-xs text-[var(--text-secondary)]">Acces immediat apres paiement. Valable 30 jours.</p>
                </div>
                <button onClick={handlePay} disabled={processing}
                  className="w-full mt-5 py-3.5 rounded-xl bg-gradient-to-r from-blue-500 to-indigo-600 text-white font-semibold shadow-lg hover:shadow-xl hover:opacity-90 transition-all disabled:opacity-60 disabled:cursor-not-allowed flex items-center justify-center gap-2">
                  {processing
