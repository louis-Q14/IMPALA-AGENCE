...
export default function FinanceRevenus() {
  ...
  const fetchRevenue = useCallback(async () => {
    try {
      ...
      const res = await fetch(`${API_BASE}/finance/revenue`, {
  ...
  const updateTxStatus = async (txId: string, status: "paid" | "pending" | "refunded") => {
    try {
      const token = typeof window !== "undefined" ? localStorage.getItem("token") : null;
      await fetch(`${API_BASE}/finance/revenue/transactions/${txId}/status`, {
        method: "PATCH",
  ...
  const exportCSV = () => {
    ...
    const rows = filtered.map((tx) => [
      ...
      tx.desc,
      tx.amount.toFixed(2).replace(".", ",") + " FC",
      tx.date,
    ...
    a.href = url;
    a.download = `revenus_finance_${new Date().toISOString().split("T")[0]}.csv`;
    a.click();
    URL.revokeObjectURL(url);
  };

  const formatAmount = (n: number) => n.toLocaleString("fr-FR", { minimumFractionDigits: 2, maximumFractionDigits: 2 }) + " FC";
  ...
  return (
    <>
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-sm text-[var(--text-muted)]">
            Analyse financière de la plateforme · {transactions.length} transactions
          </p>
        </div>
        <button
          onClick={exportCSV}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {[
          { label: "Revenu total", value: chartTotal, icon: CurrencyDollarIcon, color: "bg-amber-500", change: "+18%" },
          { label: "Immobilier", value: chartImmo, icon: HomeIcon, color: "bg-blue-500", change: "+22%" },
          { label: "Automobile", value: chartAuto, icon: TruckIcon, color: "bg-purple-500", change: "+15%" },
          ...
          <div key={card.label} className="p-6 rounded-2xl bg-[var(--bg-card)] border border-[var(--border-color)]">
            ...
            <p className="text-2xl font-bold text-[var(--text-primary)]">
              {card.value.toLocaleString("fr-FR")} FC
            </p>
      ...
      <div className="p-6 rounded-2xl bg-[var(--bg-card)] border border-[var(--border-color)] mb-8">
              ...
              <div key={d.month} className="flex items-center gap-4">
                ...
                <div className="flex-1 flex gap-0.5 h-10 rounded-lg overflow-hidden">
                  <div
                    ...
                    title={`Immobilier: ${d.immobilier.toLocaleString("fr-FR")} FC`}
                  />
                  <div
                    className="bg-purple-500 transition-all duration-500"
                    style={{ width: `${(d.auto / maxVal) * 100}%` }}
                    title={`Automobile: ${d.auto.toLocaleString("fr-FR")} FC`}
                  />
                  <div
        ...
        <div className="flex items-center gap-6 mt-6 pt-4 border-t border-[var(--border-color)]">
          ...
          <div className="flex items-center gap-2 text-sm text-[var(--text-secondary)]">
            <div className="w-3 h-3 rounded-sm bg-purple-500" /> Automobile
          </div>
          <div className="flex items-center gap-2 text-sm text-[var(--text-secondary)]">
            ...
            <div className="flex items-center gap-3 w-full sm:w-auto">
              <div className="relative flex-1 sm:flex-none">
                ...
                <input
                  ...
                  className="w-full sm:w-56 pl-9 pr-4 py-2 rounded-xl bg-[var(--bg-tertiary)] border border-[var(--border-color)] text-sm text-[var(--text-primary)] placeholder:text-[var(--text-muted)] focus:outline-none focus:ring-2 focus:ring-amber-500/50"
                />
              </div>
              <button
            ...
            <div className="flex flex-wrap gap-3 mt-4 pt-4 border-t border-[var(--border-color)]">
              <div>
                ...
                <select
                  ...
                  className="px-3 py-2 rounded-xl bg-[var(--bg-tertiary)] border border-[var(--border-color)] text-sm text-[var(--text-primary)] focus:outline-none focus:ring-2 focus:ring-amber-500/50"
                >
              ...
              <thead>
                <tr className="border-b border-[var(--border-color)] bg-[var(--bg-tertiary)]">
                  {["Facture", "Client", "Service", "Description", "Montant", "Date", "Statut", "Actions"].map((h) => (
                    <th key={h} className="text-left text-xs font-medium text-[var(--text-muted)] uppercase tracking-wider px-4 py-3">
                      {h}
                    </th>
                  ))}
                </tr>
              ...
              <tbody className="divide-y divide-[var(--border-color)]">
                {filtered.map((tx) => (
                  <tr key={tx.id} className="hover:bg-[var(--bg-hover)] transition-all">
                    <td className="px-4 py-3 text-sm font-mono text-amber-600">{tx.id}</td>
                    <td className="px-4 py-3 text-sm text-[var(--text-primary)]">{tx.user}</td>
                    <td className="px-4 py-3">
                      <div className={`w-7 h-7 rounded-md flex items-center justify-center ${
                        tx.service === "real_estate" ? "bg-blue-100 dark:bg-blue-900/30"
                          : tx.service === "auto" ? "bg-purple-100 dark:bg-purple-900/30"
                          : "bg-emerald-100 dark:bg-emerald-900/30"
                      }`}>
                        {tx.service === "real_estate" && <HomeIcon className="w-4 h-4 text-blue-500" />}
                        {tx.service === "auto" && <TruckIcon className="w-4 h-4 text-purple-500" />}
                    ...
                    <td className="px-4 py-3 text-sm font-medium text-[var(--text-primary)]">{formatAmount(tx.amount)}</td>
                    <td className="px-4 py-3 text-sm text-[var(--text-muted)]">{tx.date}</td>
                    <td className="px-4 py-3">
                      <span className={`px-2.5 py-1 rounded-full text-xs font-medium ${
                        tx.status === "paid" ? "bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400"
                    ...
                    <td className="px-4 py-3">
                      <div className="flex gap-1">
                        <button
                          ...
                          title="Détails"
                          className="w-8 h-8 rounded-lg bg-blue-100 dark:bg-blue-900/30 flex items-center justify-center text-blue-600 hover:bg-blue-200 transition-all"
                        >
                        ...
                        {tx.status === "pending" && (
                          <button
                            ...
                            title="Marquer payé"
                            className="w-8 h-8 rounded-lg bg-emerald-100 dark:bg-emerald-900/30 flex items-center justify-center text-emerald-600 hover:bg-emerald-200 transition-all"
                          >
                        ...
                        {tx.status === "paid" && (
                          <button
                            ...
                            title="Rembourser"
                            className="w-8 h-8 rounded-lg bg-red-100 dark:bg-red-900/30 flex items-center justify-center text-red-600 hover:bg-red-200 transition-all"
                          >
          ...
          <div className="bg-[var(--bg-card)] rounded-2xl border border-[var(--border-color)] w-full max-w-lg p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-start justify-between mb-6">
              <div className="flex items-center gap-3">
                <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${
                  selectedTx.service === "real_estate" ? "bg-blue-100 dark:bg-blue-900/30"
                    : selectedTx.service === "auto" ? "bg-purple-100 dark:bg-purple-900/30"
                    : "bg-emerald-100 dark:bg-emerald-900/30"
                }`}>
                  {selectedTx.service === "real_estate" && <HomeIcon className="w-6 h-6 text-blue-500" />}
                  {selectedTx.service === "auto" && <TruckIcon className="w-6 h-6 text-purple-500" />}
              ...
              <button onClick={() => setSelectedTx(null)} className="w-8 h-8 rounded-lg hover:bg-[var(--bg-hover)] flex items-center justify-center transition-all">
            ...
            <div className="space-y-3 mb-6">
              {[
                ...
                <div key={item.label} className="flex justify-between items-center py-2 border-b border-[var(--border-color)]">
            ...
            <div className="flex items-center gap-2 mb-6">
              ...
              <span className={`px-2.5 py-1 rounded-full text-xs font-medium ${
                selectedTx.status === "paid" ? "bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400"
                  : selectedTx.status === "pending" ? "bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400"
                  : "bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400"
              }`}>
            ...
            <div className="flex gap-3">
              {selectedTx.status === "pending" && (
                <button onClick={() => markAsPaid(selectedTx.id)} className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-emerald-600 text-white font-medium hover:bg-emerald-700 transition-all text-sm">
                  <CheckCircleIcon className="w-4 h-4" /> Marquer comme payé
                </button>
              )}
              {selectedTx.status === "paid" && (
                <button onClick={() => setRefundConfirm(selectedTx)} className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-red-600 text-white font-medium hover:bg-red-700 transition-all text-sm">
                  <ArrowPathIcon className="w-4 h-4" /> Rembourser
                </button>
              )}
              <button onClick={() => setSelectedTx(null)} className="px-6 py-2.5 rounded-xl border border-[var(--border-color)] text-sm font-medium text-[var(--text-secondary)] hover:bg-[var(--bg-hover)] transition-all">
                Fermer
              </button>
            </div>
          </div>
      ...
      {refundConfirm && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4" onClick={() => setRefundConfirm(null)}>
          <div className="bg-[var(--bg-card)] rounded-2xl border border-[var(--border-color)] w-full max-w-md p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <div className="flex flex-col items-center text-center mb-6">
              ...
              <p className="text-sm text-[var(--text-muted)]">
                ...
                <span className="font-semibold text-[var(--text-primary)]">{formatAmount(refundConfirm.amount)}</span>{" "}
                à <span className="font-semibold text-[var(--text-primary)]">{refundConfirm.user}</span>.
              </p>
              <p className="text-sm text-red-500 dark:text-red-400 mt-3 font-medium">Cette action est irréversible.</p>
            </div>
            <div className="flex gap-3">
              <button onClick={() => setRefundConfirm(null)} className="flex-1 px-4 py-2.5 rounded-xl border border-[var(--border-color)] text-sm font-medium text-[var(--text-secondary)] hover:bg-[var(--bg-hover)] transition-all">
                Annuler
              </button>
              <button onClick={() => refundTransaction(refundConfirm)} className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-red-600 text-white font-medium hover:bg-red-700 transition-all text-sm">
                <ArrowPathIcon className="w-4 h-4" /> Rembourser
              </button>
            </div>
          </div>
